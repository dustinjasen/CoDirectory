    ...
    success: function (result) {
      ...
      if ($(document).width() > 990) {
        ...
        for (var i = 0; i < allData.length; i++) {
          ...
          let col7 = "<td scope='row' class='eAddress'><a style='color: #D03800;' href='mailto:" + eMail + "'>" + eMail + "</a></td>";
        ...
        for (var j = 0; j < allData.length; j++) {
          ...
          let col8 = "<td scope='row'><a style='color: #D03800;' href='mailto:" + eMail + "'><i class='far fa-envelope'></i></a></td>";
      ...
      $(".clickRow").click(function () {
        ...
        var previewDeptID = row.find(".dID").val();
        ...
        if ($(document).width() > 990) {
          ...
          let editDeptDiv = '<div class="mb-3 form-group"><label for="editDeptDesktop" class="labels">Change Department from ' + previewDept + '</label><select class="form-select" aria-label="editDeptDesktop" name="editDeptDesktop" id="editDeptDesktop" value="' + previewDeptID + '"></select><div class="col-sm-5 messages"></div></div>';
          ...
          $("#deleteProfilebtn").click(function (e) {
            ...
            var previewFirstName = $('#editFirstNameDesktop').val();
            //var deleteYes = deleteProfileAJAX(profileID);
      ...
      $('#addUserDesktopPanel').click(function () {
        ...
        let heading = '<div class="createContactHeader">Create Contact<button class="btn exitButton float-end" onClick="cancelButton()"><i class="fas fa-share-square"></i></button></div>';
        ...
        let cancelNewUser = '<button type="button" class="btn cancelButton" onClick="cancelButton()" data-bs-dismiss="modal">Cancel</button>';
        let saveNewUser = '<button type="submit" class="btn saveButton">Create</button>';
...
function allDeptTable() {
  $.ajax({
    ...
    success: function (result) {
      ...
      let heading = '<div class="DeptAdminHeader"><button class="btn addButton float-start" id="newDepartmentButton"><i class="fas fa-plus-circle"></i></button>Department Admin<button class="btn exitButton float-end" onClick="cancelButton()"><i class="fas fa-share-square"></i></button></div>';
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        let col1 = "<tr><td class='deptNameEdit clickRowDept'>" + name + "</td>";
        let col2 = "<td class='d-none deptAdminID'>" + deptID + "</td>";
        let col3 = "<td class='d-none deptLocationAdminID '>" + locID + "</td>";
        let delBtn = '<td><button class="deleteDepartment deleteButtonTwo"><i class="far fa-trash-alt"></i></button></td></tr>';
      ...
      $('#newDepartmentButton').click(function () {
        ...
        let headingNewDept = '<div class="createContactHeader">Create Department<button class="btn exitButton float-end" onClick="cancelButton()"><i class="fas fa-share-square"></i></button></div>';
        ...
        let newDepartmentCancel = '<button type="button" class="btn cancelButton" onClick="cancelButton()">Cancel</button>';

        let newDepartmentFormEnd = '</form>';

        populateLocationforDeptOptions();

        $('#previewData').html(headingNewDept + newDepartmentFormStart + newDepartmentInput + newDepartmentLocationInput + newDepartmentSave + newDepartmentCancel + newDepartmentFormEnd);
      ...
      $('.clickRowDept').click(function () {
        ...
        let deptNameF = row.find("td:eq(0)").text();
        let departmentIdF = row.find("td:eq(1)").text();
        let locationIdF = row.find("td:eq(2)").text();
        ...
        let editDeptID = '<p class="d-none" id="editDeptID">' + departmentIdF + '</p>';
        let editDeptName = '<div class="mb-3 form-group"><label for="editDeptName" class="labels">Edit Name</label><input class="form-control" name="editDeptName" id="editDeptName" value="' + deptNameF + '" required></input><div class="col-sm-5 messages"></div></div>';

        let EditDepartmentLocation = '<div class="mb-3 form-group"><label for="editDepartmentLocation" class="labels">Change Location</label><select class="form-select" name="editDepartmentLocation" id="editDepartmentLocation" value="' + locationIdF + '" required></select><div class="col-sm-5 messages"></div></div>';
        ...
        $('#previewData').append(headingDeptEdt + editFormStart + editDeptID + editDeptName + EditDepartmentLocation + saveBtn + cancelBtn + editFormEnd);
        populateLocationforDeptOptions();
...
function allLocationsTable() {
  $.ajax({
    ...
    success: function (result) {
      ...
      let heading = '<div class="LocationAdminHeader"><button class="btn addButton float-start" id="addNewLocation"><i class="fas fa-plus-circle"></i></button>Location Admin<button class="btn exitButton float-end" onClick="cancelButton()"><i class="fas fa-share-square"></i></button></div>';
      ...
      for (var i = 0; i < allData.length; i++) {
        ...
        let delBtn = '<td><button class="deleteLocation deleteButtonTwo" ><i class="far fa-trash-alt"></i></button></td></tr>';
      ...
      $('.clickRowLoc').click(function () {
        ...
        let row = $(this).closest("tr");
        let locationNameF = row.find("td:eq(0)").text();
        ...
        let editLocationID = '<p class="d-none" id="editLocationID">' + locationIdF + '</p>';
        let editLocationName = '<input class="form-input" name="editLocationName" id="editLocationName" value="' + locationNameF + '" required></input><div class="col-sm-5 messages"></div>';
        let saveBtn = '<div><button type="submit" class="btn saveButton">Save</button>';
        let cancelBtn = '<button type="button" class="btn cancelButton" onClick="cancelButton()" data-bs-dismiss="modal">Cancel</button></div>';
        let formEnd = '</form>'
      ...
      $('#addNewLocation').click(function () {
        ...
        let headingLocNew = '<div class="createContactHeader">Create Location<button class="btn exitButton float-end" onClick="cancelButton()"><i class="fas fa-share-square"></i></button></div>';
        ...
        let newLocationCancel = '<button type="button" class="btn cancelButton" onClick="cancelButton()">Cancel</button>';
        let newLocationFormEnd = '</form>';

        $('#previewData').html(headingLocNew + newLocationFormStart + newLocationInput + newLocationSave + newLocationCancel + newLocationFormEnd);
